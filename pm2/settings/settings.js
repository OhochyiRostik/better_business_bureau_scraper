...
if (process.platform === 'win32') {
  PYTHON_INTERPRETER = path.join(_projectDirectory, '..', 'venv', 'Scripts', 'python.exe');
  SCRAPY_SCRIPT = path.join(_projectDirectory, '..', 'venv', 'Scripts', 'scrapy.exe');
} else {
  PYTHON_INTERPRETER = path.join(_projectDirectory, '..', 'venv', 'bin', 'python');
  SCRAPY_SCRIPT = path.join(_projectDirectory, '..', 'venv', 'bin', 'scrapy');
}

if (!fs.existsSync(PYTHON_INTERPRETER)) {
  console.error('ERROR: python(virtualenv) not found');
  console.error(PYTHON_INTERPRETER);
  process.exit(1);
}
if (!fs.existsSync(PYTHON_INTERPRETER)) {
